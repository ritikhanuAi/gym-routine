// ---------- Data ----------

const DAYS = [
  {
    id: "mon", dow: "Monday", short: "MON", code: "PUSH A",
    title: "Chest + Shoulders + Triceps",
    focus: "Main focus: Chest",
    accent: "--push-a",
    exercises: [
      ["Barbell Bench Press", "3 × 6–10"],
      ["Incline Dumbbell Press", "3 × 8–12"],
      ["Cable Fly", "2 × 12–15"],
      ["Dumbbell Lateral Raise", "3 × 12–20"],
      ["Cable Triceps Pushdown", "3 × 10–15"],
      ["Overhead Cable Triceps Extension", "2 × 10–15"]
    ]
  },
  {
    id: "tue", dow: "Tuesday", short: "TUE", code: "PULL A",
    title: "Back + Biceps + Rear Delts",
    focus: "Main focus: Back thickness + overall back",
    accent: "--pull-a",
    exercises: [
      ["Lat Pulldown", "3 × 8–12"],
      ["Barbell Row", "3 × 6–10"],
      ["Seated Cable Row", "3 × 8–12"],
      ["Face Pull", "3 × 12–20"],
      ["Dumbbell Curl", "3 × 8–12"],
      ["Hammer Curl", "2 × 10–15"]
    ]
  },
  {
    id: "wed", dow: "Wednesday", short: "WED", code: "LEGS A",
    title: "Quad emphasis + calves + abs",
    focus: "This should be your harder quad-focused leg day.",
    accent: "--legs-a",
    exercises: [
      ["Barbell Squat", "4 × 6–10"],
      ["Leg Press", "3 × 8–12"],
      ["Bulgarian Split Squat", "3 × 8–12 ea."],
      ["Leg Extension", "2 × 12–15"],
      ["Leg Curl", "3 × 10–15"],
      ["Standing Calf Raise", "3 × 10–15"],
      ["Cable Crunch", "3 × 10–15"]
    ]
  },
  {
    id: "thu", dow: "Thursday", short: "THU", code: "PUSH B",
    title: "Shoulders + Upper Chest",
    focus: "Main focus: Shoulders + upper chest",
    accent: "--push-b",
    exercises: [
      ["Overhead Dumbbell Press", "3 × 6–10"],
      ["Incline Barbell Bench Press", "3 × 6–10"],
      ["Machine Chest Press", "3 × 8–12"],
      ["Cable Lateral Raise", "3 × 12–20"],
      ["Pec Deck", "2 × 12–15"],
      ["Rope Triceps Extension", "3 × 10–15"]
    ]
  },
  {
    id: "fri", dow: "Friday", short: "FRI", code: "PULL B",
    title: "Lats + Rear Delts + Biceps",
    focus: "Main focus: Lat width",
    accent: "--pull-b",
    exercises: [
      ["Pull-ups / Assisted Pull-ups", "3 × 6–10"],
      ["Chest-Supported Row", "3 × 8–12"],
      ["Single-Arm Lat Pulldown", "3 × 10–12"],
      ["Reverse Pec Deck", "3 × 12–20"],
      ["Incline Dumbbell Curl", "3 × 8–12"],
      ["Cable Hammer Curl", "2 × 10–15"]
    ]
  },
  {
    id: "sat", dow: "Saturday", short: "SAT", code: "LEGS B",
    title: "Hamstrings + Glutes",
    focus: "Main focus: Hamstrings + glutes",
    accent: "--legs-b",
    exercises: [
      ["Romanian Deadlift", "3 × 6–10"],
      ["Hack Squat", "3 × 8–12"],
      ["Walking Lunges", "3 × 10–12 ea."],
      ["Seated Leg Curl", "3 × 10–15"],
      ["Leg Extension", "2 × 12–15"],
      ["Seated Calf Raise", "3 × 10–15"],
      ["Hanging Knee Raise", "3 × 10–15"]
    ]
  },
  {
    id: "sun", dow: "Sunday", short: "SUN", code: "REST",
    title: "Rest / Recovery",
    rest: true
  }
];

// ---------- Weekly-reset storage ----------
// Key is namespaced by ISO year+week, so a new week always starts blank.

function isoWeekKey(date = new Date()) {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
  const dayNum = (d.getUTCDay() + 6) % 7; // Mon=0..Sun=6
  d.setUTCDate(d.getUTCDate() - dayNum + 3);
  const firstThursday = new Date(Date.UTC(d.getUTCFullYear(), 0, 4));
  const week = 1 + Math.round(
    ((d.getTime() - firstThursday.getTime()) / 86400000 - 3 + ((firstThursday.getUTCDay() + 6) % 7)) / 7
  );
  return `${d.getUTCFullYear()}-W${String(week).padStart(2, "0")}`;
}

const STORAGE_PREFIX = "training-log:";
const weekKey = isoWeekKey();
const storageKey = STORAGE_PREFIX + weekKey;

function loadState() {
  try {
    const raw = localStorage.getItem(storageKey);
    return raw ? JSON.parse(raw) : {};
  } catch {
    return {};
  }
}

function saveState(state) {
  try {
    localStorage.setItem(storageKey, JSON.stringify(state));
  } catch {
    /* storage unavailable — app still works in-memory for this session */
  }
}

// Clean up any checklist state from previous weeks so storage doesn't grow forever.
function pruneOldWeeks() {
  try {
    Object.keys(localStorage)
      .filter(k => k.startsWith(STORAGE_PREFIX) && k !== storageKey)
      .forEach(k => localStorage.removeItem(k));
  } catch {
    /* ignore */
  }
}
pruneOldWeeks();

let state = loadState();

// ---------- Render ----------

const daysEl = document.getElementById("days");
const stripEl = document.getElementById("weekStrip");
const weekLabelEl = document.getElementById("weekLabel");
const pctNumEl = document.getElementById("pctNum");

weekLabelEl.textContent = `WEEK ${weekKey.split("-W")[1]} · ${weekKey.split("-W")[0]}`;

function dayCompletion(day) {
  if (day.rest) return null;
  const done = day.exercises.filter((_, i) => state[day.id]?.[i]).length;
  return { done, total: day.exercises.length, pct: done / day.exercises.length };
}

function overallPct() {
  let done = 0, total = 0;
  DAYS.forEach(day => {
    if (day.rest) return;
    total += day.exercises.length;
    done += day.exercises.filter((_, i) => state[day.id]?.[i]).length;
  });
  return total === 0 ? 0 : Math.round((done / total) * 100);
}

function checkIcon() {
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>`;
}

function chevronIcon() {
  return `<svg class="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>`;
}

function ringSVG(pct, colorVar) {
  const r = 14, c = 2 * Math.PI * r;
  const offset = c * (1 - pct);
  return `
    <svg class="ring" viewBox="0 0 34 34">
      <circle class="bg" cx="17" cy="17" r="${r}" fill="none" stroke-width="3"/>
      <circle class="fg" cx="17" cy="17" r="${r}" fill="none" stroke-width="3"
        stroke-dasharray="${c}" stroke-dashoffset="${offset}"
        stroke-linecap="round" transform="rotate(-90 17 17)"
        style="stroke:var(${colorVar})"/>
    </svg>`;
}

function renderStrip() {
  stripEl.innerHTML = "";
  DAYS.forEach(day => {
    const plate = document.createElement("div");
    plate.className = "plate" + (day.rest ? " rest" : "");
    const c = dayCompletion(day);
    const pct = c ? Math.round(c.pct * 100) : 0;
    if (!day.rest) {
      plate.classList.toggle("done", pct === 100);
      plate.tabIndex = 0;
      plate.addEventListener("click", () => {
        document.getElementById(`card-${day.id}`)?.scrollIntoView({ block: "center" });
      });
    }
    const fillColor = day.rest ? "var(--rest)" : `var(${day.accent})`;
    plate.innerHTML = `
      <div class="plate-fill" style="width:${day.rest ? 100 : pct}%; background:${fillColor}; opacity:${day.rest ? 0.25 : 1}"></div>
      <div class="plate-label">${day.short}</div>`;
    stripEl.appendChild(plate);
  });
}

function renderDays() {
  daysEl.innerHTML = "";
  DAYS.forEach(day => {
    if (day.rest) {
      const rc = document.createElement("div");
      rc.className = "rest-card";
      rc.textContent = "SUNDAY — REST / RECOVERY";
      daysEl.appendChild(rc);
      return;
    }

    const c = dayCompletion(day);
    const card = document.createElement("section");
    card.className = "day-card";
    card.id = `card-${day.id}`;
    card.style.setProperty("--accent", `var(${day.accent})`);

    const head = document.createElement("div");
    head.className = "day-head";
    head.tabIndex = 0;
    head.innerHTML = `
      <div class="day-head-left">
        <div class="day-eyebrow">${day.dow.toUpperCase()} — ${day.code}</div>
        <div class="day-name">${day.title}</div>
      </div>
      <div class="day-head-right">
        <span class="day-frac">${c.done}/${c.total}</span>
        ${ringSVG(c.pct, day.accent)}
        ${chevronIcon()}
      </div>`;
    head.addEventListener("click", () => card.classList.toggle("collapsed"));

    const list = document.createElement("ul");
    list.className = "exercise-list";
    day.exercises.forEach(([name, reps], i) => {
      const li = document.createElement("li");
      const checked = !!state[day.id]?.[i];
      li.className = "exercise" + (checked ? " checked" : "");
      li.tabIndex = 0;
      li.innerHTML = `
        <span class="box">${checkIcon()}</span>
        <span class="ex-name">${name}</span>
        <span class="ex-reps">${reps}</span>`;
      const toggle = () => {
        state[day.id] = state[day.id] || {};
        state[day.id][i] = !state[day.id][i];
        saveState(state);
        renderAll();
      };
      li.addEventListener("click", toggle);
      li.addEventListener("keydown", e => {
        if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggle(); }
      });
      list.appendChild(li);
    });

    const focus = document.createElement("div");
    focus.className = "day-focus";
    focus.textContent = day.focus;

    card.appendChild(head);
    card.appendChild(list);
    card.appendChild(focus);
    daysEl.appendChild(card);
  });
}

function renderAll() {
  renderStrip();
  renderDays();
  pctNumEl.textContent = overallPct();
}

renderAll();

// ---------- Reset ----------

document.getElementById("resetBtn").addEventListener("click", () => {
  if (!confirm("Reset all checkmarks for this week?")) return;
  state = {};
  saveState(state);
  renderAll();
});
